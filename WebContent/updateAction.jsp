...
<%
	...
	if(!userID.equals(bbs.getUserID())){//userID(세션에 있는 값)과 getUserID(이 글을 작성한 값)을 서로 비교해서 동일하지 않을 경우(동일한경우 문제 없음)
		...
		if(request.getParameter("bbsTitle") == null || request.getParameter("bbsContent") == null
					...
					||request.getParameter("bbsContent").equals("")){//"bbsTitle" 이름으로 매개변수로 넘어온 값들을 분석할 수 있도록 함.,update.jsp에서  name에서 매개변수로 넘어온다.
				    PrintWriter script = response.getWriter();
				    script.println("<script>"); 
				...
				}else{// 정상적으로 다 입력이 되었다면 성공적으로 수정할 수 있음.
					BbsDAO bbsDAO = new BbsDAO();//데이터베이스에 접근할 수 있는 객체
					int result = bbsDAO.update(bbsID, request.getParameter("bbsTitle"), request.getParameter("bbsContent"));//write 함수를 실행해서 실제로 게시글을 작성할 수 있도록 함.,각각의 매개변수 넣어줌

					if(result == -1){ //데이터베이스 오류 발생한 경우
					    PrintWriter script = response.getWriter();
					    script.println("<script>"); 
					    script.println("alert('글수정에 실패했습니다.') "); //데이터베이스 오류시 글쓰기 실패
					    script.println("history.back() "); 
					    script.println("</script>");              
					 }
